...
use Closure;
use Losingbattle\MicroBase\Request\Traits\ScopedValidatorsTrait;
use Generator;
...
trait BaseValidateTrait
{
     ...
     * @param null|bool $stopOnError Stop verification if there is an error
     *
    ...
    {
        if ($this->scene && $conf = $this->getScenarios()) {
    ...
    protected function applyRule($fields, array $rule, array $onlyChecked, bool $stopOnError): void
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($filters && $value !== null && ! strpos($field, '.*')) {
     ...
     * @param Closure|mixed|string $validator Validator
